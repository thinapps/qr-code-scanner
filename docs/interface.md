...
The scanner screen includes a visual-only aiming guide above the camera preview. It uses a preferred maximum size of `260dp` and can shrink on cramped layouts so it does not crowd the torch button, screen edges, or bottom scanner panel. The guide uses four open corners instead of a full box, so it gives users an aiming reference without making the camera view feel boxed in.
