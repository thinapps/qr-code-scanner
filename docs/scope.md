...
Selected-image scanning lets ML Kit load the chosen image directly from the picker URI, then accepts the first non-blank raw value only and preserves it without trimming or rewriting it. It does not add cropping, editing, image preview, multi-select, batch processing, or a separate result flow.
...
Scanning happens locally on the device. Camera permission is requested only for live scanning. Selected-image scanning uses the system picker, requires no storage or broad photo permission, lets ML Kit read the returned URI directly, and does not copy or retain the selected image in app storage.
...
Selected-image fallback preprocessing is also intentionally out of scope. The app uses ML Kit's normal URI-based image loading and will not retry mirrored copies, inverted-color copies, contrast or brightness adjustments, sharpening, thresholding, rescaling passes, or other transformed versions after scanning fails. Multiple fallback transformations and decoder passes would add memory use, delay, branches, and testing for rare edge cases. One specific fallback should be reconsidered only if repeated real-world reports prove it is needed.
